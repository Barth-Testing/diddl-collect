...
function leseProfilFrisch(): Record<string, number> {
  if (typeof window === "undefined") return {};
  try {
    return JSON.parse(window.localStorage.getItem(PROFIL_FRISCH_KEY) ?? "{}") as Record<string, number>;
  } catch {
    return {};
  }
}

function merkeProfilFrisch(id: string) {
  if (typeof window === "undefined") return;
  const staende = leseProfilFrisch();
  staende[id] = Date.now();
  window.localStorage.setItem(PROFIL_FRISCH_KEY, JSON.stringify(staende));
}

const fremdeProfileLaden = new Set<string>();

export async function ladeFremdesProfil(id: string): Promise<boolean> {
  ...
  try {
    ...
    if (idx >= 0) {
      if (JSON.stringify(users[idx]) === JSON.stringify(benutzer)) {
        merkeProfilFrisch(id);
        return false;
      }
      users[idx] = benutzer;
    } else {
