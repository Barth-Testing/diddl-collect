...
> 4. Regressionstest vor jedem Deploy, der Store/Sync/Supabase anfasst: Gerät A markiert Blatt → Gerät B (frische Seite, KEIN Login) muss die Markierung ohne Neuanmeldung sehen. **Egress-Sparmodus (06.09.2026):** Der Eigen-Poll prüft zuerst nur `profile.updated_at` (Mini-Check, `scripts/updated-at-sync.sql` – Spalte+Trigger+Grant anlegen!) und lädt die volle Zeile nur bei Änderung → Cross-Device bleibt bei ~20 s, ohne MB-Egress. Ohne die Spalte fällt er auf vollen Abruf alle 2 min zurück (Refresh/Fokus laden trotzdem sofort). Wenn nur Re-Login hilft → Lese-Pfad ist kaputt.
...
- Schema changes are applied **manually by the user in the Supabase SQL editor** — hand them ready-to-run SQL statements. Deploys can go live BEFORE the SQL runs, so any new column/function needs a graceful fallback in code (catch `PGRST204`/`42703`, `PGRST202` für fehlende Funktionen → alter Weg; siehe `favoritenUnterstuetzt` in store.ts, PGRST202-Fallbacks in login/register). **Niemals `create or replace function` mit geänderter Signatur** – das erzeugt eine stille Überladung statt Ersatz (Alt-Clients bekommen `300 Multiple Choices` und loopen im Retry → Egress-Sturm 09/2026 bei `profil_patch`); alte Signatur immer per `drop function (typen…)` entfernen (siehe `profil-patch-antwort.sql`).
...
- **Blatt-IDs in Nutzerdaten sind FREIGESCHALTET (Remap bei jedem Lese-Pfad).** Die 12 alten Sammelverzeichnis-IDs (`A5-463` … `A6-234`, am 27.08.2026 auf `diddlback-de-*` umgezogen) stehen in `ALTE_BLATT_IDS` (types.ts); `normalisiereStatusesKatalog` + `remappeBlattSchluesselKatalog` (blaetter.ts) mappen statuses/beweise/favoriten/tausch beim Laden. **Der Remap ist KATALOGBEWUSST:** Existiert (wieder) ein Katalog-Blatt mit einer alten ID (z. B. klassisches `A6-230`, 2006), gewinnt das Katalog-Blatt – die ID wird dann NICHT remappt (sonst wirken neue Karten unmarkierbar, Markierungen wandern aufs Diddl-is-Back-Blatt). Die SQL-Migration `scripts/didlback-id-migration.sql` zieht auch die DB um. **NIEMALS Katalog-Blätter umbenennen/umbetten** – ein ID-Wechsel macht alle Nutzer-Markierungen (und Tausch-Offerten) unsichtbar. Neue Blätter bekommen neue IDs, alte bleiben stabil; eine Wiederverwendung einer alten ID ist dank katalogbewusstem Remap abgesichert.
